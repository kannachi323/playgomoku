  ...
  persist<AuthState>(
    (set, get) => ({
      ...
      signup: async (email, password) => {
        const res = await fetch(`http://${import.meta.env.VITE_ROOT}/signup`, {
            method: "POST",
            headers: {
      ...
      login: async (email, password) => {
        const res = await fetch(`http://${import.meta.env.VITE_ROOT}/login`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
      ...
      logout: async (callback: () => void | Promise<void>) => {
        const res = await fetch(`http://${import.meta.env.VITE_ROOT}/logout`, {
          method: "GET",
          credentials: "include",
        });
      ...
      checkAuth: async (onFail: () => void | Promise<void>) => {
        ...
        try {
          setAuthLoading(true);
          const res = await fetch(`http://${import.meta.env.VITE_ROOT}/check-auth`, {
            method: 'GET',
            credentials: 'include',
          })
          if (res.ok) {
